...
class Mongo:
    ...
    def bulk_insert(
        self, collection_name: str, data: list[dict[str, Any]], ordered: bool = False
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"Bulk inserted {result.inserted_count} documents into {collection_name}"
            )
            return result
        except Exception as e:
    ...
    def bulk_update(
        self, collection_name: str, updates: list[dict[str, Any]], ordered: bool = False
    ):
        """
        ...
        operations = [
            UpdateOne(
                update["filter"], update["update"], upsert=update.get("upsert", False)
            )
        ...
        try:
            result = collection.bulk_write(operations, ordered=ordered)
            logger.info(
                f"Bulk updated {result.modified_count} documents in {collection_name}"
            )
            return result
        except Exception as e:
    ...
    def safe_get_metric_dict(self, obj, metric_name: str, default=None):
        ...
        try:
            ...
            return {
                "total": (
                    self._distribution_summary_to_dict(total)
                    if total is not None
                    else default
                ),
                "successful": (
                    self._distribution_summary_to_dict(successful)
                    if successful is not None
                    else default
                ),
                "errored": (
                    self._distribution_summary_to_dict(errored)
                    if errored is not None
                    else default
                ),
            }
        except (AttributeError, TypeError):
            return {"total": default, "successful": default, "errored": default}
