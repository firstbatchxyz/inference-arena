...
from utils.sglang_utils import (
    ...
    get_optimal_sglang_config,
    get_sglang_environment_vars,
)


def create_sglang_pod(
    gpu_id: str,
    volume_in_gb: int,
    ...
    print(
        f"Time taken to start the SGLang server: {time_taken_to_start_sglang_server_seconds}"
    )
    ...
    pod_details = {
        ...
        "gpu_count": gpu_count,
        "is_gpt_oss": "gpt-oss" in llm_id.lower()
        or "gpt_oss" in llm_id.lower(),  # Add GPT-OSS flag
        "docker_image": image_name,  # Add the Docker image used
    }
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=30000, help="Port number (default: 30000)"
    )
    parser.add_argument(
