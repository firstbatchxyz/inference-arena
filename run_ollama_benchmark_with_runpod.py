...
def create_ollama_pod(
    ...
    print(
        f"Time taken to start the ollama server: {time_taken_to_start_ollama_server_seconds}"
    )
    ...
    client = GuideLLMBenchmarkClient(
        ...
        max_retries=5,  # Increase retries for Ollama
        base_delay=3.0,  # Longer base delay
        max_delay=120.0,  # Longer max delay
        timeout=600.0,  # Much longer timeout for Ollama
        health_check_timeout=90.0,  # Longer health check timeout
    )
    ...
    concurrent_rates = list(range(1, 5))  # Test rates 1-4 (Ollama is typically slower)

    suite_results = client.run_benchmark_suite(
        pod_id=pod_id,
        concurrent_rates=concurrent_rates,
        include_throughput=True,
        max_seconds_per_test=30,  # Longer tests for Ollama
    )
    ...
    if suite_results["successful_tests"] == 0:
        ...
        for benchmark_report_rate in range(4):  # Reduced from 6 to 4
            try:
                report, path = client.run_sync_with_retry(
                    max_seconds=30,  # Increased from 15 to 30
                    mongo_query={},
