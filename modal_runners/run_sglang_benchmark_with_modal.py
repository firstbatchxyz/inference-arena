...
# Modal image setup for SGLang
sglang_image = (
    modal.Image.from_registry("nvidia/cuda:12.8.0-devel-ubuntu22.04", add_python="3.11")
    .entrypoint([])
    .apt_install("libnuma-dev")  # Required for sgl_kernel
    .uv_pip_install(
        "sglang[all]>=0.5.2rc0",
...
async def create_sglang_modal_server(
    llm_id: str,
    port: int = 8000,
    ...
    def serve_sglang():
        ...
        from utils.sglang_utils import build_sglang_serve_args
        
        # Build SGLang serve arguments
        # build_sglang_serve_args returns: "model_id --port X --host Y ..."
        # We need to convert to: "--model-path model_id --port X --host Y ..."
        serve_args_str = build_sglang_serve_args(llm_id, port, gpu_count)
        parsed_args = shlex.split(serve_args_str)
        
        if parsed_args:
            model_id_from_args = parsed_args[0]
            other_args = parsed_args[1:]
            
            # Build command: python -m sglang.launch_server --model-path <model_id> <other_args>
            cmd = ["python", "-m", "sglang.launch_server", "--model-path", model_id_from_args] + other_args
            
            print(f"Starting SGLang with command: {' '.join(cmd)}")
            
            # Start SGLang server - Modal's web_server decorator keeps container alive
            subprocess.Popen(cmd)
        else:
            raise ValueError(f"Failed to parse serve args: {serve_args_str}")
    ...
    with deployment_app.run():
        ...
        web_url = serve_sglang.get_web_url()
        print(f"Modal server deployed at: {web_url}")
        
        # Wait for SGLang server to be ready using async HTTP 
        max_wait_time = 10 * MINUTES
        start_wait = time.time()
        print(f"Waiting for SGLang server to be ready ...")
        
        async with aiohttp.ClientSession() as session:
            while time.time() - start_wait < max_wait_time:
                try:
                    # Check if SGLang server is ready by verifying model is loaded
                    async with session.get(f"{web_url}/v1/models", timeout=aiohttp.ClientTimeout(total=30)) as models_resp:
                        if models_resp.status == 200:
                            ...
                            if models_data.get("data") and any(
                                ...
                                print(f"SGLang server is ready at {web_url} with model {llm_id}")
        ...
        time_taken_to_start_sglang_server = (
            sglang_server_readiness_time - start_time
        )
        ...
        server_id = f"modal-sglang-{int(time.time())}"
